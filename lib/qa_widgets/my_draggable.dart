...
/// Makes its child MyDraggable starting from long press.
class ImmediatePressMyDraggable<T> extends MyDraggable<T> {
  /// Creates a widget that can be dragged starting from long press.
  ///
  /// The [child] and [feedback] arguments must not be null. If
  /// [maxSimultaneousDrags] is non-null, it must be non-negative.
  const ImmediatePressMyDraggable({
    Key key,
    @required Widget child,
    @required Widget feedback,
    T data,
...
class _MyDragTargetState<T> extends State<MyDragTarget<T>> {
  ...
  void didLeave(DragAvatar<T> avatar) {
  ...
  void didDrop(DragAvatar<T> avatar) {
    assert(_candidateAvatars.contains(avatar));
    if (!mounted)
      return;
    setState(() {
...
// seeding this object pointer events even after it has been disposed.
class DragAvatar<T> extends Drag {
  ...
  Iterable<_MyDragTargetState<T>> _getMyDragTargets(Iterable<HitTestEntry> path) sync* {
  ...
  void finishDrag(_DragEndKind endKind, [Velocity velocity]) {
    ...
    _entry.remove();
    _entry = null;
    // TODO: consider passing _entry as well so the client can perform an animation.
    if (onDragEnd != null)
